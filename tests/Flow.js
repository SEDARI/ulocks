...
/*	    describe("operation lub", function() {
		it("with identical targets and locks",function() {
		    var f1 = new Flow({ target : { type: "any" }, locks : [ { path : "inTimePeriod", args : [ "10:00", "11:00" ] }, { path : "hasId", args : [ "1", "2" ] } ] });
		    ...
		    expect(JSON.stringify(newFlow) === JSON.stringify(f2)).toBe(true);
		...
		it("with identical targets and overlapping time locks",function() {
		    ...
		    expect(JSON.stringify(newFlow1) === JSON.stringify(lub_f1_f2)).toBe(true);
		    expect(res2).toBe(true);
		    expect(JSON.stringify(newFlow2) === JSON.stringify(lub_f1_f2)).toBe(true);
		});
